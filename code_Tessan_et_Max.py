...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        sp500_button = ttk.Button(self, text="S&P 500", command=lambda: controller.show_frame(SP500))
        sp500_button.pack()
        
        btc_button = ttk.Button(self, text="Bitcoin", command=lambda: controller.show_frame(BtcApp))
        btc_button.pack()
...
class SP500(tk.Frame):
    # Figure where the graph will be
    ...
    def __init__(self,parent, controller):
        ...
        # define if the webdriver has beed launched : initial mode = not launched
        self.launched = False
        # define if the animation is running or not
        self.running = False
        
        # Button to start and pause the graph  : will launch the webdriver and start the animation thanks to start_pause function
        self.start_pause_button = tk.ttk.Button(top_frame, text="Start", command=lambda: self.start_pause())
    ...
    def start_pause(self):
        ...
        if not self.launched :
            self.launched = True
            self.driver_SP500 = webdriver.Firefox()
            self.driver_SP500.get("https://www.cnbc.com/quotes/?symbol=.SPX")
            sleep(4)
        
            
        # init the animation :
        if self.ani is None:      #si l'animation n'est pas lancée alors on retourn self.start()
            return self.start()
        
        
        if self.running: #si l'animation est en pause alors on la passe en start-mode en appuyant sur le bouton 
    ...
    def animate(self, interval):
        ...
        market_value = str(self.driver_SP500.find_element_by_xpath("/html/body/div[4]/div/div[3]/div[3]/div[1]/div[2]/div/div[1]/div/table/tbody/tr[5]/td[1]/span[1]").text)
...
class BtcApp(tk.Frame):
     ...
     def __init__(self, parent, controller):
        ...
        # Set threads:
        self.btc_thread = threading.Thread(target = self.on_click)
        ...
        self.btn = tk.Button(btns, text='Start', command=self.thread_manager)
     ...
     def thread_manager(self):
        # lancer le thread si le driver n'est pas activé
        if not self.launched:
            self.btc_thread.start()
        # si le driver est lancé on close le thread et on rentre direct dans la fonction    
        else:
